...
def _register_psx() -> None:
    from .psx import service as psx

    async def probe():
        snapshot = await psx.get_stocks_snapshot()
        return {
            "ready": snapshot.count > 0,
            "records": snapshot.count,
            "state": snapshot.freshness(get_settings().psx_stale_after_s)["state"],
    ...
    system.register_dataset("psx", probe, refresh)
...
def create_app(domains: Iterable[str] = ("psx", "mufap"), *,
    ...
    async def lifespan(app: FastAPI):
        ...
        if settings.scheduler_enabled:
            from .scheduler import Scheduler, build_mufap_worker, build_psx_worker

            scheduler = Scheduler()
            if "psx" in domains:
                scheduler.spawn("psx", build_psx_worker())
            if "mufap" in domains:
    ...
    if "psx" in domains:
        from .psx.router import router as psx_router
    ...
    # Static frontend is mounted last so it cannot shadow an API route.
    if static_dir and settings.serve_static and static_dir.is_dir():
        app.mount("/", StaticFiles(directory=str(static_dir), html=True), name="static")
        logger.info("static_mounted", extra={"path": str(static_dir)})
