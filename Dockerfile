...
HEALTHCHECK --interval=30s --timeout=5s --retries=3 --start-period=15s \
  CMD curl --fail --silent --max-time 4 "http://127.0.0.1:${PORT:-8000}/health" || exit 1

# exec form so uvicorn is PID 1 and receives SIGTERM directly.
CMD ["sh", "-c", "exec uvicorn app.main:app --host 0.0.0.0 --port ${PORT:-8000} --workers 1 --timeout-graceful-shutdown 20"]
