...
| **Coverage** | 1,073 listed PSX instruments · 18 indices · 550 mutual funds |
...
Every path below is a plain `GET` returning JSON. There is no key, no quota and
no rate limit: the files sit on GitHub's CDN, so a request costs nothing and
never touches PSX or MUFAP. Verified live on 2026-09-17 — all 100 files 200 OK.

| What a consumer needs | Value |
|---|---|
| CORS | `Access-Control-Allow-Origin: *` — callable from any browser origin |
| Compression | `Content-Encoding: gzip` — `mufap/funds.json` is 317 KB raw, **36 KB on the wire** |
| Caching | `Cache-Control: max-age=600`, plus `ETag` and `Last-Modified` for conditional requests |
| Auth | None |
| Methods | `GET` only |
...
| Path | Returns |
|---|---|
| [`index.json`](https://sohaib-sarwar.github.io/PSX-MUFAP-MicroService/api/index.json) | **The catalog.** Every endpoint, every field, both schedules. Generated by the same code that writes the data, so it cannot drift. Start here. |
| [`search.json`](https://sohaib-sarwar.github.io/PSX-MUFAP-MicroService/api/search.json) | **Search index** across both domains — 1,623 entries of `id`, `label`, `group`, `issuer`, no prices. 22 KB gzipped. See [Search](#search). |
| [`/health.json`](https://sohaib-sarwar.github.io/PSX-MUFAP-MicroService/health.json) | Liveness. At the site root, not under `/api`. |
| [`/ready.json`](https://sohaib-sarwar.github.io/PSX-MUFAP-MicroService/ready.json) | Readiness — per-dataset freshness. `status` is `ready` or `warming_up`. |

### PSX equities — `api/psx/`

| Path | Returns | Rows |
|---|---|---|
| `psx/index.json` | Domain info, source, schedule, counts | — |
| `psx/stocks.json` | **Every listed instrument**, volume-ordered. `traded: false` means listed but did not trade — not a fetch failure | 1,073 |
| `psx/stocks/summary.json` | Market breadth: advancers, decliners, volume, traded value, average move | — |
| `psx/stocks/gainers.json` | Top gainers by percentage move | 50 |
| `psx/stocks/losers.json` | Top losers by percentage move | 50 |
| `psx/stocks/active.json` | Most active by volume | 50 |
| `psx/sectors.json` | Per-sector breadth; each row carries the `slug` for the endpoint below | 38 |
| `psx/sectors/{slug}.json` | **Instruments in one sector.** 1.5 KB gzipped instead of 35 KB | per sector |
| `psx/indices.json` | Index board — KSE100, KSE30, KMI30, ALLSHR and the rest | 18 |
| `psx/market-status.json` | `open` / `closed` / `unknown`, inferred from the newest KSE100 tick rather than a hardcoded calendar | — |

Sector slugs come from `psx/sectors.json` — e.g. `commercial-banks`,
`refinery`, `technology-communication`, `cement`, `fertilizer`.

### MUFAP mutual funds — `api/mufap/`

| Path | Returns | Rows |
|---|---|---|
| `mufap/index.json` | Domain info, source, schedule, counts | — |
| `mufap/funds.json` | **Every fund** — NAV, offer/repurchase, all three loads, rating, trustee, AMC, 11 return periods | 550 |
| `mufap/funds/categories.json` | **Every category** with fund count and its `slug` | 35 |
| `mufap/funds/category/{slug}.json` | **Funds in one category.** 2.7 KB gzipped instead of 36 KB | per category |
| `mufap/funds/amcs.json` | Asset management companies with fund counts | 25 |
| `mufap/funds/stats.json` | Industry aggregates — NAV mean/median/min/max, YTD best/worst/mean | — |
| `mufap/funds/top/{period}.json` | **Best performers** over one period | 50 |

`{period}` is one of `ytd` `mtd` `d1` `d15` `d30` `d90` `d180` `d270` `d365`
`y2` `y3`. Category slugs come from `mufap/funds/categories.json` — e.g.
`money-market`, `shariah-compliant-money-market`, `equity`, `income`.

### Search

A static host cannot answer `?q=` — the query is not known when the file is
written. `search.json` is the honest equivalent: every instrument and every
fund reduced to what a match needs, plus an `endpoints` map to the files
holding the full records.
...
const hits = index.data.filter((row) =>
  [row.id, row.label, row.issuer].some((field) =>
    field?.toLowerCase().includes('habib')
  )
)
// 45 hits, first one:
// { type: 'stock', id: 'HBL', label: 'Habib Bank Limited',
//   group: 'COMMERCIAL BANKS', active: true }

// Then pull the full record for a hit:
const file = index.endpoints[hits[0].type]          // 'psx/stocks.json'
const full = await fetch(`${BASE}/${file}`).then((r) => r.json())
const record = full.data.find((row) => (row.symbol ?? row.fund_name) === hits[0].id)
```
...
```json
{
  "count": 550, "total": 550, "offset": 0, "limit": null,
  "freshness": {
    "state": "fresh",
    "data_as_of": "2026-09-18",
    "fetched_at": "2026-09-17T18:44:03+05:00",
    "stale_after_seconds": 7616,
    "next_refresh_at": "2026-09-17T19:05:00+05:00",
    "record_count": 550
  },
  "data": []
}
...
`state` is `fresh`, `stale`, `degraded` or `unavailable`. `degraded` means the
last refresh failed and you are reading the previous good snapshot — the data
is real, just not new.

`data_as_of` is what **the source** says the data is from: the last trade
timestamp for PSX, the NAV validity date for MUFAP. It is not the same as
`fetched_at`, and it is the one that matters.

`age_seconds` is measured when the file is written and **cannot tick on a
static host**. Compute the real age yourself:
...
`stale_after_seconds` is derived from the publication schedule, so it already
accounts for weekends — a Friday snapshot read on Sunday is `fresh`, not stale.

### Using it from another project

```bash
curl -s https://sohaib-sarwar.github.io/PSX-MUFAP-MicroService/api/psx/stocks/gainers.json \
  | jq '.data[0] | {symbol, name, current, change_pct}'
```
...
print(stocks[stocks.traded].nlargest(10, "volume")[["symbol", "name", "current", "change_pct"]])
```
...
where filtering and pagination happen server-side through query parameters.
The static API above is the same data with those queries precomputed into
paths.
